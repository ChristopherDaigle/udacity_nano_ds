...
# fourth chart shows rural population vs arable land
    ...
    graph_five = []
    df_five = cleandata('data/API_SP.RUR.TOTL_DS2_en_csv_v2_9914824.csv', keepcolumns =['Country Name', '2015'], value_variables=['2015'])
    df_five.columns = ['country','year','rural_population']
    df_five.sort_values('rural_population', ascending=False, inplace=True)

    graph_five.append(
        go.Bar(
        x=list(df_five['country']),
        y=list(df_five['rural_population'])
        )
    )

    layout_five = dict(title = 'Rural Population of Top-Ten Countries in 2015',
                xaxis = dict(title = 'Country'),
    ...
    # DONE: once the data is clean, make a list called graph_five and append the plotly graph to this list.

    # DONE: fill a layout variable for the fifth visualization
    ...
    # DONE: append the figure five information to the figures list
